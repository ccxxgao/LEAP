...
def all_evals():
    ...
    if request.method == "GET":
        ...
        return render_template("admin.html", evaluations=evaluations)
        ...
        part_one = db.execute(
            "SELECT * FROM part1")
        dpt = db.execute("SELECT * FROM department")
        part_twoa = db.execute("SELECT * FROM part2a")
        part_twob = db.execute("SELECT * FROM part2b")
        part_three = db.execute("SELECT * FROM part3")
        part_four = db.execute("SELECT * FROM part4")

        return render_template("admin_employee_eval.html", evaluations=evaluations, one=part_one, dpt=dpt, part_twoa=part_twoa, part_twob=part_twob, part_three=part_three, part_four=part_four)
